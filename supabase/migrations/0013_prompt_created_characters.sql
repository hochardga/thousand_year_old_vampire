...
begin
  ...
  new_character_kind_text := coalesce(nullif(new_character->>'kind', ''), 'mortal');
  ...
  if new_character_kind_text not in ('mortal', 'immortal') then
    raise exception 'A character kind must be mortal or immortal.'
      using errcode = 'P0001';
  end if;

  new_character_kind := new_character_kind_text::public.character_kind;
