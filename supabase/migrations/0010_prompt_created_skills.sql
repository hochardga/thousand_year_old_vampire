...
begin
  ...
  new_skill_description := btrim(coalesce(new_skill->>'description', ''));

  if new_skill_label = '' then
    raise exception 'A skill name is required.'
      using errcode = 'P0001';
  end if;

  if new_skill_description = '' then
    raise exception 'A skill description is required.'
      using errcode = 'P0001';
  end if;
  ...
  if exists (
    ...
    where chronicle_id = target_chronicle_id
      and btrim(label) = new_skill_label
  ) then
    raise exception 'A skill with this name already exists.'
      using errcode = 'P0001';
  end if;
  ...
  returning id into created_skill_id;

  return created_skill_id;
end;
$$;

drop function if exists public.resolve_prompt_run(uuid, uuid, text, text, jsonb, jsonb);
