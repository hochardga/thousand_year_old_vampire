create or replace function public.require_in_mind_memory(
  target_chronicle_id uuid,
  target_memory_id uuid,
  error_message text default 'Choose a memory still held in mind.'
)
...
begin
  ...
  if not found or memory_record.location <> 'mind' then
    if error_message = 'Only memories still held in mind can accept new entries.' then
      raise exception 'Only memories still held in mind can accept new entries.'
        using errcode = 'P0001';
    end if;

    raise exception 'Choose a memory still held in mind.'
      using errcode = 'P0001';
  end if;
...
create or replace function public.memory_entry_count(
  target_memory_id uuid
)
returns integer
language sql
...
create or replace function public.next_open_memory_slot(
  target_chronicle_id uuid
)
returns integer
language plpgsql
