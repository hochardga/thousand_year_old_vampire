...
begin
  ...
  if new_resource_label = '' then
    raise exception 'A resource name is required.'
      using errcode = 'P0001';
  end if;

  if new_resource_description = '' then
    raise exception 'A resource description is required.'
      using errcode = 'P0001';
  end if;
...
drop function if exists public.resolve_prompt_run(uuid, uuid, text, text, jsonb, jsonb, jsonb);
