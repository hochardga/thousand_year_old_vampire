...
| Trait changes during play | The play UI now supports prompt-created skills, resources, marks, and characters, but it still submits empty mutation arrays and offers no normal controls for most prompt-driven checks, losses, existing-character changes, or mark updates. |
| Character and mark upkeep | Existing characters and marks can be edited after the fact in the ledger, but many prompt-specific changes to existing traits are not surfaced in active play. |
...
It is not yet a full digital implementation of the rules text. The biggest genuine gaps are setup fidelity, prompt-driven mutation of existing traits, substitution logic, diary-loss consequences, and game-ending conditions.
...
| If a prompt requires a Character and none is available, create one. | Automated | The play surface now supports prompt-created mortal or immortal Characters, duplicate-name checks, and transactional prompt-resolution persistence. | `src/components/ritual/PromptCharacterComposer.tsx`, `src/components/ritual/PlaySurface.tsx`, `src/lib/validation/play.ts`, `supabase/migrations/0013_prompt_created_characters.sql`, `tests/integration/archive-rules.test.ts`, `tests/integration/setup-flow.test.tsx` |
